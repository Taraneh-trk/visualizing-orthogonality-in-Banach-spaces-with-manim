...
class TitleScene(Scene):
    ...
    def scene1(self):
        ...
        cauchy_sequence_dis = MathTex(r"\|f_i - f_j\| = \left| \, \frac{1}{i} - \frac{1}{j} \, \right|",color=dark_blue).next_to(encounterexample,DOWN)
        cauchy_sequence = MathTex(r"\text{Cauchy sequence}",color=dark_blue).scale(1.2).move_to(cauchy_sequence_dis.get_center())
        
        self.play(
            TransformFromCopy(VGroup(*[norm_example1, encounterexample]), cauchy_sequence_dis),
        )
        self.wait(0.5)
        self.play(
        ...
        self.play(
            TransformFromCopy(cauchy_sequence, convergant_sequence),
        )
        self.wait(0.5)
